...
//        database.update(DATABASE_TABLE, updatedValues, "ID = ?", new String[]{Integer.toString(ID)});
    ...
    public void deleteNote(Note note) {
        SQLiteDatabase database = this.getWritableDatabase();  //this is just in case if there is no parameters in this function
    ...
    public Cursor allNotes() {
        ...
        return database.query(DATABASE_TABLE,
                new String[]{COL_ID, COL_TITLE, COL_CONTENT/*, COL_TIMESTAMP*/},
                null,
                null,
                null,
                null,
    ...
    public Note getNoteByID (int id) {
        SQLiteDatabase database = this.getReadableDatabase();
        Cursor record = database.query(DATABASE_TABLE,
                new String[]{COL_ID, COL_TITLE, COL_CONTENT/*, COL_TIMESTAMP*/},
                "ID = ?",
                new String[]{Integer.toString(id)}, /*String.valueOf*/
                null,
                null,
                null
        );
        if(record == null || !record.moveToFirst())
            throw new IllegalArgumentException("No notes ");
...
//        LocalDateTime timestamp = LocalDateTime.parse(record.getString(3), DateTimeFormatter.ofPattern(FORMAT_DATE_TIME));
    ...
    public List<Note> findNoteWithPhrase(String phrase) {
        ...
        SQLiteDatabase database = this.getReadableDatabase();
        Cursor cursor = database.rawQuery("SELECT*FROM " + DATABASE_TABLE + " WHERE " + COL_TITLE + " LIKE ?", new String[] {"%" + phrase + "%"}, null);
        if(cursor.moveToFirst()) {
            ...
            do {
                ...
                else note.setIsBasicMode(false);
...
//                note.setTimestampNoteCreated(LocalDateTime.parse(cursor.getString(3)));
        ...
        //https://stackoverflow.com/questions/51035283/cursorindexoutofboundsexception-index-0-requested-with-a-size-of-0-in-android
        return allNotes;
    }
}